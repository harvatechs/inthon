...
def main():
    ...
    subprocess.run([sys.executable, str(benchmarks_dir / "token_efficiency.py")], check=True)
    
    print("\nRunning Workflow Correctness Benchmark...")
    subprocess.run([sys.executable, str(benchmarks_dir / "workflow_correctness.py")], check=True)
    ...
    if token_eff_path.exists():
        results["token_efficiency"] = json.loads(token_eff_path.read_text(encoding="utf-8"))
        
    wf_correct_path = benchmarks_dir / "results_workflow_correctness.json"
    if wf_correct_path.exists():
        results["workflow_correctness"] = json.loads(wf_correct_path.read_text(encoding="utf-8"))
