...
def generate_charts(suite_dir, metrics):
    ...
    rects1 = ax.bar(x - width/2, inth_times, width, label='INTHON (AST Sandboxed)', color='#1e3a8a')
    rects2 = ax.bar(x + width/2, py_times, width, label='Python (Unsandboxed)', color='#10b981')
    
    ax.set_ylabel('Execution Time (ms, Log Scale)', fontsize=9, fontweight='bold', color='#1e293b')
    ax.set_title('Execution Time Comparison (Lower is Better)', fontsize=11, fontweight='bold', pad=12, color='#1e3a8a')
    ax.set_xticks(x)
    ...
    fig, ax = plt.subplots(figsize=(8, 3.8))
    rects1 = ax.bar(x - width/2, inth_mems, width, label='INTHON (AST Sandboxed)', color='#1e3a8a')
    rects2 = ax.bar(x + width/2, py_mems, width, label='Python (Unsandboxed)', color='#10b981')
    
    ax.set_ylabel('Peak Memory Usage (MB)', fontsize=9, fontweight='bold', color='#1e293b')
    ax.set_title('Peak Memory Usage Comparison (Lower is Better)', fontsize=11, fontweight='bold', pad=12, color='#1e3a8a')
    ax.set_xticks(x)
    ax.set_xticklabels(categories, rotation=15, ha='right', fontsize=8, color='#334155')
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
...
def main():
    ...
    if not metrics_path.exists():
        print(f"[ERROR] agent_metrics.json not found at {metrics_path}. Run run_agent_benchmarks.py first.")
        return
    ...
    story.append(Paragraph("INTHON v0.1", ParagraphStyle('SubTitleBrand', fontName='Helvetica-Bold', fontSize=14, leading=18, textColor=c_secondary, spaceAfter=8)))
    story.append(Paragraph("Agent-Native Stress-Test &<br/>Performance Evaluation Report", title_style))
    
    # Decorative green bar
    story.append(Table([[""]], colWidths=[504], rowHeights=[2], style=[
    ...
    story.append(Paragraph("Evaluating Orchestration Reliability, Context Management, Schema Verification, and Sandbox Guardrails in AI-Native Programming Layers.", subtitle_style))
    ...
    story.append(Paragraph("The performance metrics from running our agent-native stress tests side-by-side against Python are detailed below.", body_style))

    # Construct Table Data
    table_data = [[
        Paragraph("<b>Stress-Test Scenario</b>", ParagraphStyle('TH', fontName='Helvetica-Bold', fontSize=8.5, leading=11, textColor=colors.white)),
        Paragraph("<b>INTHON Time</b>", ParagraphStyle('TH', fontName='Helvetica-Bold', fontSize=8.5, leading=11, textColor=colors.white, alignment=TA_RIGHT)),
        Paragraph("<b>Python Time</b>", ParagraphStyle('TH', fontName='Helvetica-Bold', fontSize=8.5, leading=11, textColor=colors.white, alignment=TA_RIGHT)),
        Paragraph("<b>INTHON Mem</b>", ParagraphStyle('TH', fontName='Helvetica-Bold', fontSize=8.5, leading=11, textColor=colors.white, alignment=TA_RIGHT)),
        Paragraph("<b>Python Mem</b>", ParagraphStyle('TH', fontName='Helvetica-Bold', fontSize=8.5, leading=11, textColor=colors.white, alignment=TA_RIGHT)),
        Paragraph("<b>Safety Guard</b>", ParagraphStyle('TH', fontName='Helvetica-Bold', fontSize=8.5, leading=11, textColor=colors.white, alignment=TA_CENTER)),
    ]]

    for key, data in metrics.items():
        table_data.append([
            Paragraph(f"<b>{data['name']}</b><br/><font size=7 color='#64748b'>{data['description']}</font>", ParagraphStyle('TD_Name', fontName='Helvetica', fontSize=8.5, leading=11)),
            Paragraph(f"{data['inthon']['time_ms']:.1f} ms" if data["inthon"]["success"] else "FAIL", ParagraphStyle('TD', fontName='Helvetica', fontSize=8.5, leading=11, alignment=TA_RIGHT)),
            Paragraph(f"{data['python']['time_ms']:.1f} ms" if data["python"]["success"] else "FAIL", ParagraphStyle('TD', fontName='Helvetica', fontSize=8.5, leading=11, alignment=TA_RIGHT)),
            Paragraph(f"{data['inthon']['memory_mb']:.1f} MB" if data["inthon"]["success"] else "FAIL", ParagraphStyle('TD', fontName='Helvetica', fontSize=8.5, leading=11, alignment=TA_RIGHT)),
            Paragraph(f"{data['python']['memory_mb']:.1f} MB" if data["python"]["success"] else "FAIL", ParagraphStyle('TD', fontName='Helvetica', fontSize=8.5, leading=11, alignment=TA_RIGHT)),
            Paragraph("VERIFIED" if data["inthon"]["success"] else "FAILED", ParagraphStyle('TD_C', fontName='Helvetica-Bold', fontSize=8.5, leading=11, alignment=TA_CENTER, textColor=c_secondary)),
        ])
    ...
    story.append(Image(str(suite_dir / "time_comparison.png"), width=6.5*inch, height=2.8*inch))
    story.append(Spacer(1, 5))
    story.append(Image(str(suite_dir / "memory_comparison.png"), width=6.5*inch, height=2.8*inch))
    story.append(PageBreak())
