...
def run_cmd_profile(cmd):
    ...
    return elapsed_ms, peak_mem, proc.returncode, stdout.decode("utf-8", errors="replace"), stderr.decode("utf-8", errors="replace")
...
def main():
    ...
    for b in benchmarks:
        ...
        if b["key"] == "infinite_loop":
            ...
            if "limit of 5 exceeded" in inth_err or "limit of 5 exceeded" in inth_out or inth_code != 0:
                inth_success = True
                print("    [GUARDRAIL INTERVENTION] Sandbox stopped infinite loop correctly.")
        ...
        if not inth_success:
            ...
            print(f"    Completed in {inth_time:.2f} ms | Peak Memory: {inth_mem:.2f} MB")
    ...
    print("="*80)
    print(f"| {'Stress-Test Scenario':<25} | {'INTHON Time':<12} | {'PYTHON Time':<12} | {'INTHON Mem':<12} | {'PYTHON Mem':<12} | {'Safety Guard':<12} |")
    ...
    for key, data in metrics.items():
        ...
        print(f"| {name:<25} | {it:<12} | {pt:<12} | {im:<12} | {pm:<12} | {guard:<12} |")
    print("="*80 + "\n")
