...
class IRBuilder(ASTVisitor):
    ...
    def build(self, program: N.Program) -> IRProgram:
        ...
        for stmt in program.body:
            if isinstance(stmt, (N.UseToolStmt, N.UsePyStmt, N.UseMemoryStmt)):
                imp = self.visit(stmt)
                if imp:
                    imports.append(imp)
            elif isinstance(stmt, N.AgentDecl):
                # Agents can also have local imports
                for imp_stmt in stmt.imports:
                    self.visit(imp_stmt)

        # Second pass: construct body
        for stmt in program.body:
            if not isinstance(stmt, (N.UseToolStmt, N.UsePyStmt, N.UseMemoryStmt)):
    ...
    def _resolve_callee(self, callee: N.Expr) -> tuple[str | None, list[str]]:
        if isinstance(callee, N.Identifier):
            return callee.name, []
        if isinstance(callee, N.MemberExpr):
            root, chain = self._resolve_callee(callee.obj)
            chain.append(callee.attr)
            return root, chain
        return None, []

    # --- Imports ---
    def visit_UseToolStmt(self, node: N.UseToolStmt) -> IRImport:
        self.imported_tools.add(node.tool_path)
        return IRImport(kind="tool", path=node.tool_path)

    def visit_UsePyStmt(self, node: N.UsePyStmt) -> IRImport:
        alias = node.alias or node.module_path.split(".")[-1]
        self.imported_py[alias] = node.module_path
        return IRImport(kind="py", path=node.module_path, alias=node.alias)

    def visit_UseMemoryStmt(self, node: N.UseMemoryStmt) -> IRImport:
        return IRImport(kind="memory", path=node.namespace)

    # --- Statements ---
    def visit_LetStmt(self, node: N.LetStmt) -> IRAssign:
        return IRAssign(target=node.name, value=self.visit(node.value))

    def visit_ConstStmt(self, node: N.ConstStmt) -> IRAssign:
        return IRAssign(target=node.name, value=self.visit(node.value))

    def visit_AssignStmt(self, node: N.AssignStmt) -> IRAssign:
        return IRAssign(target=node.target, value=self.visit(node.value))
    ...
    def visit_RecallStmt(self, node: N.RecallStmt) -> IRAssign:
        # Lower recall statement to an assignment from a recall call
        recall_call = IRCall(
            callee=IRVar("recall"),
    ...
    def visit_RetryStmt(self, node: N.RetryStmt) -> IRLoop:
        # Simple representation of retry loop in IR
        body = [self.visit(s) for s in node.body]
        return IRLoop(
            kind="retry",
            var=None,
    ...
    def visit_EvalStmt(self, node: N.EvalStmt) -> IRCall:
        # Lower eval to a call
        criteria_list: list[IRValue] = []
    ...
    def visit_UnaryOp(self, node: N.UnaryOp) -> IRBinaryOp:
        # Unary is lowered to binary operation with 0/empty or unary representation
        return IRBinaryOp(
    ...
    def visit_CallExpr(self, node: N.CallExpr) -> IRValue:
        ...
        if root:
            # Check if it is a tool call (e.g. web.search)
            ...
            # Check if it is a python module call (e.g. pd.read_csv)
    ...
    def visit_MemberExpr(self, node: N.MemberExpr) -> IRValue:
        # Non-call member expressions are parsed as variable access
    ...
    def visit_IndexExpr(self, node: N.IndexExpr) -> IRCall:
        # Lower index expression to a call to getitem
        return IRCall(
            callee=IRVar("getitem"),
            args=[self.visit(node.obj), self.visit(node.index)],
            kwargs={},
        )


def build_ir(program: N.Program) -> IRProgram:
    builder = IRBuilder()
