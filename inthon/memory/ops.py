...
def memory_remember(
    ctx, namespace: str, value: InthonValue, span: Optional[Span] = None
):
    ...
    if ctx.tracer is not None:
        ctx.tracer.emit(
            "remember", span, namespace=namespace, key=key, preview=text[:120]
        )
    return entry


def memory_recall(
    ctx, namespace: str, query: str, span: Optional[Span] = None
) -> InthonValue:
    store = ctx.memory_store_for(namespace)
    ...
    if ctx.tracer is not None:
        ctx.tracer.emit(
            ...
            query=query,
            found=bool(results),
            preview=(results[0].text[:120] if results else ""),
        )
    if not results:
        return NONE
