...
def wrap_python_exception(
    exc: BaseException, where: str, span: Optional[Span] = None
) -> InthonPyRuntimeError:
    name = type(exc).__name__
    message = str(exc) or name
    hint = None
    if isinstance(exc, (FileNotFoundError,)):
        hint = (
            "Check the path; filesystem access also requires the filesystem capability."
        )
