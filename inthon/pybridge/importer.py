...
def _wrap_callable(fn: Any, source: str) -> Any:
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        unpacked_args = [to_python(a) if isinstance(a, InthonValue) else a for a in args]
        unpacked_kwargs = {k: (to_python(v) if isinstance(v, InthonValue) else v) for k, v in kwargs.items()}
        try:
