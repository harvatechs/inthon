...
from .values import (
    ...
    InthonNone, InthonList, InthonDict, InthonCallable, InthonToolRef, InthonPyObject,
...
class Interpreter(ASTVisitor):
    ...
    def _assign_target(self, target_str: str, val: InthonValue) -> None:
        ...
        if not root_match:
            raise IntHonRuntimeError(f"INTHON_RUNTIME_ASSIGN: Invalid assignment target root '{target_str}'")
        ...
        while pos < len(target_str):
            if target_str[pos] == '.':
                ...
                if not member_match:
                    raise IntHonRuntimeError(f"INTHON_RUNTIME_ASSIGN: Invalid member in target '{target_str}' at pos {pos}")
                ...
                while pos < len(target_str) and bracket_count > 0:
                    ...
                    pos += 1
                if bracket_count > 0:
                    raise IntHonRuntimeError(f"INTHON_RUNTIME_ASSIGN: Unmatched brackets in target '{target_str}'")
                ...
                if inner.isdigit():
                    ...
                    if self._ctx.has_var(inner):
                        ...
                        raise IntHonRuntimeError(f"INTHON_RUNTIME_ASSIGN: Undefined variable '{inner}' in bracket index")
                parts.append(('index', idx_val))
            else:
                raise IntHonRuntimeError(f"INTHON_RUNTIME_ASSIGN: Invalid character '{target_str[pos]}' in target '{target_str}'")
        ...
        for i, (kind, key) in enumerate(parts[:-1]):
            if kind == 'attr':
                if isinstance(obj, InthonPyObject):
                    ...
                    raise IntHonRuntimeError(f"INTHON_RUNTIME_ASSIGN: Attribute '{key}' not found on '{type(obj)}'")
            elif kind == 'index':
                if isinstance(obj, InthonList):
                    ...
                    raise IntHonRuntimeError(f"INTHON_RUNTIME_ASSIGN: Subscript lookup failed on '{type(obj)}'")
        ...
        if last_kind == 'attr':
            if isinstance(obj, InthonPyObject):
                ...
                raise IntHonRuntimeError(f"INTHON_RUNTIME_ASSIGN: Cannot set attribute on '{type(obj)}'")
            ...
            if isinstance(obj, InthonList):
                ...
                raise IntHonRuntimeError(f"INTHON_RUNTIME_ASSIGN: Cannot set subscript on '{type(obj)}'")
    ...
    def visit_FnDecl(self, node: N.FnDecl) -> None:
        fn = InthonCallable(
            ...
            defaults={p.name: self.visit(p.default) for p in node.params if p.default is not None},
            body=node.body,
            closure=self._ctx
        )
    ...
    def visit_IfStmt(self, node: N.IfStmt) -> InthonValue:
        ...
        self._ctx.push_scope()
        try:
            branch = node.then_branch if self._is_truthy(cond) else (node.else_branch or ())
            result: InthonValue = InthonNone()
            for stmt in branch:
                val = self.visit(stmt)
                if val is not None:
    ...
    def visit_ForStmt(self, node: N.ForStmt) -> InthonValue:
        ...
        if isinstance(iterable_val, InthonList):
        ...
        elif isinstance(iterable_val, InthonPyObject) and hasattr(iterable_val.obj, '__iter__'):
            ...
            raise IntHonRuntimeError(f"INTHON_RUNTIME_FOR: Object '{type(iterable_val)}' is not iterable")
    ...
    def visit_ApproveStmt(self, node: N.ApproveStmt) -> None:
        self._ctx.policy.check_capability(Capability.PAYMENT_EXECUTE if node.action == "pay" else Capability.NETWORK)
    ...
    def visit_ForgetStmt(self, node: N.ForgetStmt) -> None:
        ...
        self._ctx.tracer.emit("forget", {"key": str(key_val), "namespace": node.namespace, "success": success})
    ...
    def visit_GuardStmt(self, node: N.GuardStmt) -> None:
        cond = self.visit(node.condition)
        if not self._is_truthy(cond):
            raise IntHonRuntimeError(f"INTHON_RUNTIME_GUARD: Guard condition failed", node.span)
    ...
    def visit_RetryStmt(self, node: N.RetryStmt) -> InthonValue:
        ...
        for attempt in range(node.count):
            try:
                ...
                self._ctx.tracer.emit("retry", {"attempt": attempt + 1, "wait_sec": wait, "error": str(e)})
                time.sleep(wait)
    ...
    def visit_EvalStmt(self, node: N.EvalStmt) -> None:
        for crit in node.criteria:
            ...
            l = to_python(metric_val)
            r = to_python(threshold_val)
            ops = {
                "==": lambda a, b: a == b,
                "!=": lambda a, b: a != b,
                "<":  lambda a, b: a < b,
            ...
            if crit.op not in ops:
                raise IntHonRuntimeError(f"INTHON_RUNTIME_EVAL: Unknown operator '{crit.op}'")
            if not ops[crit.op](l, r):
                raise IntHonRuntimeError(f"INTHON_RUNTIME_EVAL: Criterion '{crit.metric}' failed: expected {crit.op} {r}, got {l}")
    ...
    def visit_DictExpr(self, node: N.DictExpr) -> InthonDict:
        return InthonDict({to_python(self.visit(k)): self.visit(v) for k, v in node.pairs})
    ...
    def visit_UnaryOp(self, node: N.UnaryOp) -> InthonValue:
        ...
        raise IntHonRuntimeError(f"INTHON_RUNTIME_002: Unknown unary operator '{node.op}'")
    ...
    def visit_MemberExpr(self, node: N.MemberExpr) -> InthonValue:
        ...
        raise IntHonRuntimeError(f"INTHON_RUNTIME_MEMBER: Member '{node.attr}' not found on '{type(obj)}'")
    ...
    def visit_IndexExpr(self, node: N.IndexExpr) -> InthonValue:
        ...
        raise IntHonRuntimeError(f"INTHON_RUNTIME_INDEX: Index lookup failed on '{type(obj)}'")

    # ─── Private Call Methods ──────────────────────────────────────────────── #
    def _call_function(self, fn: InthonCallable, args: list[InthonValue], kwargs: dict[str, InthonValue]) -> InthonValue:
        self._ctx.push_scope()
        for i, param in enumerate(fn.params):
            if i < len(args):
                ...
                raise IntHonRuntimeError(f"INTHON_RUNTIME_004: Missing argument '{param}' for function '{fn.name}'")
    ...
    def _call_tool(self, tool_path: str, args: list[InthonValue], kwargs: dict[str, InthonValue]) -> InthonValue:
        ...
        if not result.success:
            raise ToolCallError(f"INTHON_TOOL_004: Tool execution failed: {result.error}")
    ...
    def _call_python(self, callee: InthonPyObject, args: list[InthonValue], kwargs: dict[str, InthonValue]) -> InthonValue:
        self._ctx.py_call_count += 1
        self._ctx.tracer.emit("py_call", {
            "module": callee.source_module,
            "func": callee.obj.__name__ if hasattr(callee.obj, "__name__") else str(callee.obj),
