...
class InthonStr:
    v: str
    def __repr__(self) -> str: return repr(self.v)

@dataclass
class InthonBool:
    v: bool
    def __repr__(self) -> str: return "true" if self.v else "false"
...
class InthonList:
    ...
    def __repr__(self) -> str: return f"[{', '.join(repr(i) for i in self.items)}]"

@dataclass
class InthonDict:
    pairs: dict[str, InthonValue]
    def __repr__(self) -> str: return f"{{{', '.join(f'{repr(k)}: {repr(v)}' for k, v in self.pairs.items())}}}"
...
class InthonCallable:
    ...
    def __repr__(self) -> str: return f"<function {self.name}>"

@dataclass
class InthonToolRef:
    """Reference to a registered tool (not yet called)."""
    tool_path: str
    def __repr__(self) -> str: return f"<tool {self.tool_path}>"
...
class InthonPyObject:
    ...
    obj: Any
    source_module: str
    def __repr__(self) -> str: return f"<IntHon PyObject: {self.source_module}.{type(self.obj).__name__}>"
...
def from_python(val: Any, source_module: str = "unknown") -> InthonValue:
    ...
    match val:
        ...
        case dict():
            return InthonDict({str(k): from_python(v, source_module) for k, v in val.items()})
        case InthonInt() | InthonFloat() | InthonStr() | InthonBool() | InthonNone() | InthonList() | InthonDict() | InthonCallable() | InthonToolRef() | InthonPyObject():
            return val
        case _:
