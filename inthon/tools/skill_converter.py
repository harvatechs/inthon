...
def convert_skill_to_workflow(skill_dir: Path, output_dir: Path | None = None) -> tuple[Path, Path]:
    """
    ...
    # Add Agent block
    agent_name = "".join(part.capitalize() for part in sanitized_skill_name.split("_")) + "Agent"
    ...
    for t in tools_list:
        ...
        inth_lines.append(f"        let res_{t['name'].split('.')[-1]} = {t['name']}({args_str})")
