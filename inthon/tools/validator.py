...
def validate_tool_args(spec: ToolSpec, args: list[Any], kwargs: dict[str, Any]) -> dict[str, Any]:
    ...
    for k, v in kwargs.items():
        if k in merged_kwargs:
            raise ToolCallError(f"INTHON_TOOL_003: Multiple values for argument '{k}' in tool '{spec.name}'")
        if k not in param_names:
            raise ToolCallError(f"INTHON_TOOL_003: Unexpected argument '{k}' for tool '{spec.name}'")
        merged_kwargs[k] = v
    ...
    try:
        ...
        raise ToolCallError(f"INTHON_TOOL_003: Schema validation failed for tool '{spec.name}': {e}")
