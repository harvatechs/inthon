...
def broadcast(msg: dict) -> None:
    ...
    session_state.loop.call_soon_threadsafe(lambda: asyncio.create_task(do_broadcast()))
...
def status_start(label: str) -> str:
    ...
    comp = {"id": status_id, "type": "status", "label": label, "state": "running"}
    add_component(comp)
    return status_id
...
def ui_approval_handler(req: ApprovalRequest) -> bool:
    ...
    evt = threading.Event()
    app_info = {"event": evt, "approved": False, "request": req}
...
def get_memory_items() -> list[dict]:
    ...
    try:
        ...
        if isinstance(ctx.memory, InMemoryStore):
            ...
            cursor.execute(
                "SELECT id, namespace, content_text FROM memory_entries ORDER BY created_at DESC"
            )
        ...
        while True:
            ...
            if event == "chat_message":
                ...
                if text_msg and session_state.chat_callback:
                    asyncio.create_task(
                        run_callback_in_thread(session_state.chat_callback, text_msg)
                    )
